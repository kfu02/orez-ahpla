...
epochs = 10
batch_size = 64
hidden_layer = 512 #maybe decrease to 256 and increase layer num?
def create_model():
    #nnet structure:
    ...
    conv_block_1 = Activation('relu')(BatchNormalization(axis=3)(Conv2D(hidden_layer, 3, padding='same', data_format="channels_last", use_bias=False)(input_state)))
    conv_block_2 = Activation('relu')(BatchNormalization(axis=3)(Conv2D(hidden_layer, 3, padding='same', use_bias=False)(conv_block_1)))
    conv_block_3 = Activation('relu')(BatchNormalization(axis=3)(Conv2D(hidden_layer, 3, padding='same', use_bias=False)(conv_block_2)))
    conv_block_4 = Activation('relu')(BatchNormalization(axis=3)(Conv2D(hidden_layer, 3, padding='same', use_bias=False)(conv_block_3)))
    conv_block_5 = Activation('relu')(BatchNormalization(axis=3)(Conv2D(hidden_layer, 3, padding='same', use_bias=False)(conv_block_4)))
    #flatten so data is usable
    flat = Flatten()(conv_block_5)
    #prevent overfitting
    drop_norm_1 = Dropout(dropout_rate)(Activation('relu')(BatchNormalization(axis=1)(Dense(1024, use_bias=False)(flat))))
    drop_norm_1 = Dropout(dropout_rate)(Activation('relu')(BatchNormalization(axis=1)(Dense(512, use_bias=False)(drop_norm_1))))
    #outputs
    ...
    model = Model(inputs=input_state, outputs=[pi, v])
    model.compile(loss=['categorical_crossentropy','mean_squared_error'], optimizer=Adam(alpha))
...
def train_model(model, examples):
    ...
    for state, pi, v in examples:
        ...
        x_boards.append(state_to_arr(refl_state))
        y_pis.append(np.array(refl_pi))
        y_vs.append(np.array(v))
    model.fit(x=np.array(x_boards), y=[y_pis, y_vs], batch_size=batch_size, epochs=epochs)

#takes state, returns probs, eval
def get_model_eval(model, state):
    #start_time = time.time()
    board = state_to_arr(state)
    #print(board)
    pi, v = model.predict(board.reshape(1,8,8,1)) #batch size of 1
    #print(pi, v)
...
def save_model(model, folder='saved_nnets', filename="latest_weights.h5"):
    ...
    model.save_weights(filepath) #just saves weights bc architecture is defined in init

#loads weights from folder/filename
def load_model(model, folder='saved_nnets', filename="latest_weights.h5"):
    if not filename.endswith(".h5"):
        filename += ".h5"
    filepath = os.path.join(folder, filename)
    ...
    model.load_weights(filepath) #not sure if should just save weights or not
